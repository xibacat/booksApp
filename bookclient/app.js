...
const booksURL = "https://eu-west-2.aws.data.mongodb-api.com/app/application-0-xugxs/endpoint/books";
...
document.addEventListener('load', refreshBooks(booksURL));

//Get all the books:
function refreshBooks(URL) {
    fetch(URL)
        .then(res => res.json())
        .then(json => createBookslayout(json));
}



function createBookslayout(books) {
    let cadenaDOM = "";
    books.forEach(book => {
        cadenaDOM +=
            `<div>
